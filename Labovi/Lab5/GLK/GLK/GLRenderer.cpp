...
CGLRenderer::CGLRenderer(void)
{
	...
	boja1.SetAmbient(0.2, 0.0, 0.2, 0.0);
	boja1.SetDiffuse(1.0, 0.0, 1.0, 0.0);
	boja1.SetSpecular(0.5, 0.5, 0.5, 0.0);
	boja1.SetShininess(25.0);

	boja2.SetAmbient(0.0, 0.2, 0.2, 0.0);
	boja2.SetDiffuse(0.0, 1.0, 1.0, 0.0);
	boja2.SetSpecular(0.5, 0.5, 0.5, 0.0);
	boja2.SetShininess(16.0);
	...
	//svetlo 2 sa cs
	svetlo2.SetAmbient(1.0, 0.0, 0.0, 1); //na zid na f2 
	svetlo2.SetDiffuse(1.0, 0.0, 0.0, 0); //boja kuglice
	svetlo2.SetSpecular(1, 0, 0, 1);
	svetlo2.SetEmission(1, 0, 0, 1);
	svetlo2.SetShininess(0);

	//plava - svetlo 3 odozgo sa cs
	gornjeSvetlo.SetAmbient(0.0, 0.0, 0.2, 1);//na f3 svetlo odozgo
	gornjeSvetlo.SetDiffuse(0.0, 0.0, 1.0, 0);
	gornjeSvetlo.SetSpecular(0, 0, 1, 1);
	gornjeSvetlo.SetEmission(0, 0, 1, 1);
	gornjeSvetlo.SetShininess(0);

	//svetlo 1 sa cs
	svetlo1.SetAmbient(0.2, 0, 0.2, 1);//NA F1
	svetlo1.SetDiffuse(1.0, 0.0, 1.0, 0);
	svetlo1.SetSpecular(1, 0, 1, 1);
	svetlo1.SetEmission(1, 0, 1, 1);
	svetlo1.SetShininess(0);
}
...
void CGLRenderer::Reshape(CDC* pDC, int w, int h)
{
	...
	glLoadIdentity();
	gluPerspective(65, (double)w / (double)h, 0.1, 100);
...
void CGLRenderer::DrawVase()
{
	...
	//vezba
	//int niz[14] = { 0,-3,-1,-1,-2,-1,1,3,-1,-3,-1,3,-1,-3 };
	...
	//moj sa cs
	int niz[14] = { 0, -3,-2,-1,1,-1,-1,3,-1, -3,-1, -1 ,-1, 3 };
...
void CGLRenderer::DrawRing(float radius1, float radius2, float height)
{
	...
	for (int i = 0; i < 180; i++)
	{
		...
		float stranica = fabs(radius1 - radius2);
...
void CGLRenderer::Svetlo1()
{
	/*float light_ambient[] = { 0.25, 0.0, 0.5, 1.0 };
	float light_diffuse[] = { 1, 0, 1, 1.0 };
...
void CGLRenderer::GornjeSvetlo()
{
	/*float light_ambient[] = { 0.0, 0.0, 0.5, 1.0 };
