...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_UP && m_glRenderer.angleVertical + 10 <= 90)
	{
		m_glRenderer.angleVertical += 10;
	}
	if (nChar == VK_DOWN && m_glRenderer.angleVertical - 10 >= -10)
	{
		m_glRenderer.angleVertical -= 10;
	}
	if (nChar == VK_OEM_PLUS && m_glRenderer.radius/1.1 >= 2)
	{
		m_glRenderer.radius /= 1.1;
	}
	if (nChar == VK_OEM_MINUS && m_glRenderer.radius * 1.1 <= 50)
	{
		m_glRenderer.radius *= 1.1;
	}
	if (nChar == 'W')
	{
		m_glRenderer.angleMoon += 10;
	}
	if (nChar == 'Q')
	{
		m_glRenderer.angleMoon -= 10;
