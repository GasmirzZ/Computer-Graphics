...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_RIGHT)
	{
		m_glRenderer.angleHorizontal += 10;
	}
	if (nChar == VK_LEFT)
	{
		m_glRenderer.angleHorizontal -= 10;
	}
	if (nChar == VK_UP && m_glRenderer.angleVertical<= 80)
	{
		m_glRenderer.angleVertical+= 10;
	}
	if (nChar == VK_DOWN && m_glRenderer.angleVertical>= -10)
	{
		m_glRenderer.angleVertical-= 10;
	}
	if (nChar == VK_OEM_PLUS && m_glRenderer.radius >= 14)
	{
		m_glRenderer.radius -= 6;
	}
	if (nChar == VK_OEM_MINUS && m_glRenderer.radius <= 44)
	{
		m_glRenderer.radius += 6;
	}
	if (nChar == '1')
	{
		m_glRenderer.angle1 -= 10;
	}
	if (nChar == '2')
	{
		m_glRenderer.angle1 += 10;
