...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_RIGHT)
	{
		m_glRenderer.m_angleHorizontal += 10;
	}
	if (nChar == VK_LEFT)
	{
		m_glRenderer.m_angleHorizontal -= 10;
	...
	if (nChar == VK_UP)
	{
		if (m_glRenderer.m_angleVertical <= 80)
			m_glRenderer.m_angleVertical += 10;
		else
			m_glRenderer.m_angleVertical = 90;
	}
	if (nChar == VK_DOWN)
	{
		if(m_glRenderer.m_angleVertical >= 10)
			m_glRenderer.m_angleVertical -= 10;
	}
	if (nChar == VK_ADD)
	{
		if (m_glRenderer.m_radius >= 12)
			m_glRenderer.m_radius -= 4;
		else
			m_glRenderer.m_radius = 8;
	}
	if (nChar == VK_SUBTRACT)
	{
		if (m_glRenderer.m_radius <= 46)
			m_glRenderer.m_radius += 4;
		else
			m_glRenderer.m_radius = 50;
	}
	Invalidate();
